...
// const targetSequence = ["a", "b", "c", "d", "e", "f"];
...
//   instructionText.textContent = "Tap the chests in order: a → b → c → d → e → f";
...
//     chest.setAttribute("gltf-model", "treasure_chest/scene.gltf");
//     chest.setAttribute("scale", "0.025 0.025 0.025");
...
function createGameScene() {
  ...
  const numChests = SYMBOLS.length;
  // const angleStep = (2 * Math.PI) / numChests; 
  ...
  SYMBOLS.forEach((sym, i) => {
    ...
    const chest = document.createElement("a-entity");
    chest.setAttribute("gltf-model", "../assets/treasure_chest/scene.gltf");
    chest.setAttribute("scale", isMobile ? "0.05 0.05 0.05" : "0.025 0.025 0.025");
    chest.setAttribute("position", "0 0 0");
    chest.setAttribute("rotation", "0 180 0");
    wrapper.appendChild(chest);
    ...
    collider.setAttribute("class", "clickable");
    collider.setAttribute("position", "0 1.3 0");
    collider.setAttribute("scale", isMobile ? "8 6 5" : "6 5 4"); 
    collider.setAttribute("material", "opacity: 0; transparent: true");

    collider.addEventListener("click", (e) => {
      if (isMobile && e.type === 'touchstart') e.preventDefault();
    ...
    // // Debug sphere (remove in prod: comment out)
    // const debugSphere = document.createElement("a-sphere");
    // debugSphere.setAttribute("position", "0 2.5 0");
    // debugSphere.setAttribute("radius", "0.2");
...
function enlarge(entity) {
  ...
  entity.setAttribute('animation__enlarge', {
    property: 'scale',
    to: isMobile ? '0.6 0.6 0.6' : '0.3 0.3 0.3', 
    dur: 200,
    dir: 'alternate',
    loop: 1
  });
...
function shake(entity) {
  ...
  const shakeDuration = 50;
  const shakeMagnitude = isMobile ? 0.1 : 0.05;
  let delay = 0;
